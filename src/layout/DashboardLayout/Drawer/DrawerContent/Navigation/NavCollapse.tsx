...
// assets
import { CheckBoxOutlineBlankOutlined, KeyboardArrowDown, KeyboardArrowUp, KeyboardArrowRight } from '@mui/icons-material';
...
const NavCollapse = ({ menu, level, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: NavCollapseProp) => {
  ...
  const handleClickCollapse = (event: any) => {
  ...
  const handleClick = (event: any, isRedirect: boolean) => {
    setAnchorEl(null);
  ...
  const borderIcon = level === 1 ? <CheckBoxOutlineBlankOutlined style={{ fontSize: '1rem' }} /> : false;
          ...
          >
            ...
            {(drawerOpen || (!drawerOpen && level !== 1)) &&
              (menu?.url ? (
                ...
                >
                  {miniMenuOpened || open ? (
                    <KeyboardArrowUp style={{ fontSize: '0.625rem', color: theme.palette.primary.main }} />
                  ) : (
                    <KeyboardArrowDown style={{ fontSize: '0.625rem' }} />
                  )}
                </IconButton>
              ) : (
                <>
                  {miniMenuOpened || open ? (
                    <KeyboardArrowUp style={{ fontSize: '0.625rem', marginLeft: 1, color: theme.palette.primary.main }} />
                  ) : (
                    <KeyboardArrowDown style={{ fontSize: '0.625rem', marginLeft: 1 }} />
                  )}
                </>
              ))}
        ...
        <>
          ...
          >
            <Box onClick={handlerIconLink} sx={FlexBox}>
              ...
              />
              {miniMenuOpened ? <KeyboardArrowRight /> : <KeyboardArrowDown />}
            </Box>
